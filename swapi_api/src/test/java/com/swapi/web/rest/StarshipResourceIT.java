...
import static com.swapi.domain.StarshipAsserts.*;
...
import com.swapi.domain.Starship;
...
/**
 * Integration tests for the {@link StarshipResource} REST controller.
 */
@IntegrationTest
@AutoConfigureMockMvc
@WithMockUser
class StarshipResourceIT {
    ...
    private static final Double DEFAULT_HYPERDRIVE_RATING = 1D;
    private static final Double UPDATED_HYPERDRIVE_RATING = 2D;

    private static final Integer DEFAULT_MGLT = 1;
    private static final Integer UPDATED_MGLT = 2;

    private static final String DEFAULT_STARTSHIP_CLASS = "AAAAAAAAAA";
    private static final String UPDATED_STARTSHIP_CLASS = "BBBBBBBBBB";

    private static final String ENTITY_API_URL = "/api/starships";
    ...
    @Autowired
    private MockMvc restStarshipMockMvc;

    private Starship starship;

    private Starship insertedStarship;
     ...
     */
    public static Starship createEntity() {
        return new Starship()
            ...
            .hyperdriveRating(DEFAULT_HYPERDRIVE_RATING)
            .mglt(DEFAULT_MGLT)
            .startshipClass(DEFAULT_STARTSHIP_CLASS);
    }
     ...
     * if they test an entity which requires the current entity.
     */
    public static Starship createUpdatedEntity() {
        return new Starship()
            ...
            .hyperdriveRating(UPDATED_HYPERDRIVE_RATING)
            .mglt(UPDATED_MGLT)
            .startshipClass(UPDATED_STARTSHIP_CLASS);
    }

    @BeforeEach
    void initTest() {
        starship = createEntity();
    }

    @AfterEach
    void cleanup() {
        if (insertedStarship != null) {
            //starshipRepository.delete(insertedStarship);
            insertedStarship = null;
        }
    }

    @Test
    @Transactional
    void getAllStarships() throws Exception {
        // Initialize the database
        //insertedStarship = starshipRepository.saveAndFlush(starship);

        // Get all the starshipList
        restStarshipMockMvc
            .perform(get(ENTITY_API_URL + "?sort=id,desc"))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(starship.getId().intValue())))
            ...
            .andExpect(jsonPath("$.[*].hyperdriveRating").value(hasItem(DEFAULT_HYPERDRIVE_RATING)))
            .andExpect(jsonPath("$.[*].mglt").value(hasItem(DEFAULT_MGLT)))
            .andExpect(jsonPath("$.[*].startshipClass").value(hasItem(DEFAULT_STARTSHIP_CLASS)));
    }

    @Test
    @Transactional
    void getStarship() throws Exception {
        // Initialize the database
        //insertedStarship = starshipRepository.saveAndFlush(starship);

        // Get the starship
        restStarshipMockMvc
            .perform(get(ENTITY_API_URL_ID, starship.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.id").value(starship.getId().intValue()))
            ...
            .andExpect(jsonPath("$.hyperdriveRating").value(DEFAULT_HYPERDRIVE_RATING))
            .andExpect(jsonPath("$.mglt").value(DEFAULT_MGLT))
            .andExpect(jsonPath("$.startshipClass").value(DEFAULT_STARTSHIP_CLASS));
    }

    @Test
    @Transactional
    void getNonExistingStarship() throws Exception {
        // Get the starship
        restStarshipMockMvc.perform(get(ENTITY_API_URL_ID, Long.MAX_VALUE)).andExpect(status().isNotFound());
    }
